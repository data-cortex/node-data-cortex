...
test('EVENT_PROP_LIST is combination of string, number, and other props', () => {
  const expectedProps = [...STRING_PROP_LIST, ...NUMBER_PROP_LIST, ...OTHER_PROP_LIST];
  
  assert.deepStrictEqual(EVENT_PROP_LIST, expectedProps);
  assert.strictEqual(EVENT_PROP_LIST.length, STRING_PROP_LIST.length + NUMBER_PROP_LIST.length + OTHER_PROP_LIST.length);
});
...
test('BUNDLE_PROP_LIST is combination of event and default bundle props', () => {
  ...
  assert.strictEqual(BUNDLE_PROP_LIST.length, EVENT_PROP_LIST.length + DEFAULT_BUNDLE_PROP_LIST.length);
});
...
test('LOG_PROP_LIST is combination of log number, string, and other props', () => {
  const expectedProps = LOG_NUMBER_PROP_LIST.concat(Object.keys(LOG_STRING_PROP_MAP), LOG_OTHER_PROP_LIST);
  
  assert.deepStrictEqual(LOG_PROP_LIST, expectedProps);
  assert.strictEqual(LOG_PROP_LIST.length, LOG_NUMBER_PROP_LIST.length + Object.keys(LOG_STRING_PROP_MAP).length + LOG_OTHER_PROP_LIST.length);
});
...
test('no overlap between STRING_PROP_LIST and NUMBER_PROP_LIST', () => {
  ...
  STRING_PROP_LIST.forEach(prop => {
    assert.strictEqual(numberSet.has(prop), false, `Property ${prop} should not be in both string and number lists`);
  });
  
  NUMBER_PROP_LIST.forEach(prop => {
    assert.strictEqual(stringSet.has(prop), false, `Property ${prop} should not be in both string and number lists`);
  });
});
...
test('no overlap between STRING_PROP_LIST and OTHER_PROP_LIST', () => {
  ...
  STRING_PROP_LIST.forEach(prop => {
    assert.strictEqual(otherSet.has(prop), false, `Property ${prop} should not be in both string and other lists`);
  });
  
  OTHER_PROP_LIST.forEach(prop => {
    assert.strictEqual(stringSet.has(prop), false, `Property ${prop} should not be in both string and other lists`);
  });
});
...
test('no overlap between NUMBER_PROP_LIST and OTHER_PROP_LIST', () => {
  ...
  NUMBER_PROP_LIST.forEach(prop => {
    assert.strictEqual(otherSet.has(prop), false, `Property ${prop} should not be in both number and other lists`);
  });
  
  OTHER_PROP_LIST.forEach(prop => {
    assert.strictEqual(numberSet.has(prop), false, `Property ${prop} should not be in both number and other lists`);
  });
});
