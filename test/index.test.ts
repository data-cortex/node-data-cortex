...
test('exports bound setDeviceTag method', () => {
  ...
  assert.strictEqual(dataCortex.defaultObject.defaultBundle.device_tag, 'test_device');
});
...
test('exports bound setUserTag method', () => {
  ...
  assert.strictEqual(dataCortex.defaultObject.defaultBundle.user_tag, 'test_user');
});
...
test('exports bound flush method', () => {
  ...
  dataCortex.defaultObject._sendEvents = function() { sendEventsCalled = true; };
  dataCortex.defaultObject._sendLogs = function() { sendLogsCalled = true; };
...
test('exports bound install method', () => {
  ...
  dataCortex.init({ apiKey: API_KEY, orgName: ORG_NAME, deviceTag: 'device123' });
...
test('exports bound dau method', () => {
  ...
  const events = dataCortex.defaultObject.eventList.filter(e => e.type === 'dau');
...
test('exports bound event method', () => {
  ...
  const events = dataCortex.defaultObject.eventList.filter(e => e.type === 'event');
  assert.strictEqual(events.length, 1);
});
...
test('exports bound economy method', () => {
  ...
  dataCortex.init({ apiKey: API_KEY, orgName: ORG_NAME, deviceTag: 'device123' });
  dataCortex.economy({ spend_currency: 'USD', spend_amount: 10.50 });
  
  const events = dataCortex.defaultObject.eventList.filter(e => e.type === 'economy');
  assert.strictEqual(events.length, 1);
  assert.strictEqual(events[0].spend_currency, 'USD');
  assert.strictEqual(events[0].spend_amount, 10.50);
});
...
test('exports bound messageSend method', () => {
  ...
  dataCortex.init({ apiKey: API_KEY, orgName: ORG_NAME, deviceTag: 'device123' });
  dataCortex.messageSend({ network: 'email', from_tag: 'sender', to_list: ['recipient'] });
  
  const events = dataCortex.defaultObject.eventList.filter(e => e.type === 'message_send');
  assert.strictEqual(events.length, 1);
});
...
test('exports bound messageClick method', () => {
  ...
  dataCortex.init({ apiKey: API_KEY, orgName: ORG_NAME, deviceTag: 'device123' });
  dataCortex.messageClick({ network: 'email', from_tag: 'sender', to_tag: 'recipient' });
  
  const events = dataCortex.defaultObject.eventList.filter(e => e.type === 'message_click');
  assert.strictEqual(events.length, 1);
});
...
test('bound methods work independently of defaultObject reference', () => {
  ...
  newInstance.init({ apiKey: API_KEY, orgName: ORG_NAME, deviceTag: 'device123' });
  
  // The bound methods should still work with the original defaultObject
  dataCortex.init({ apiKey: API_KEY, orgName: ORG_NAME, deviceTag: 'device456' });
  dataCortex.event({ kingdom: 'bound_test' });
  
  // Check that the bound method affected the defaultObject, not the new instance
  assert.strictEqual(dataCortex.defaultObject.eventList.length, 1);
  assert.strictEqual(dataCortex.defaultObject.eventList[0].kingdom, 'bound_test');
